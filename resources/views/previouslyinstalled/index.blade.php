...
<section class="">
    <div class="content p-4">
        <div class="row pt-3">
            <div class="col-md-6">
                <h3 class=""><?php echo trans('lang.previouslyinstalled_list');?></h3>
            </div>
            <div class="col-md-6 text-md-right pb-md-0 pb-3">
            @if (count(app('userAccess')('PREVIOUSLYINSTALLED_ADD')) > 0)
    ...
    <!--edit new data -->
    @if (count(app('userAccess')('PREVIOUSLYINSTALLED_EDIT')) > 0)
    ...
    <!--delete data -->
    @if (count(app('userAccess')('PREVIOUSLYINSTALLED_DELETE')) > 0)
...
$("#formadd").validate({
    submitHandler: function(form) {
        $.ajax({
			method: "POST",
            url: "{{ url('savepreviouslyinstalled')}}",
...
$("#formedit").validate({
    submitHandler: function(form) {
        $.ajax({
			method: "POST",
            url: "{{ url('updatepreviouslyinstalled')}}",
...
$("#formdelete").validate({
    submitHandler: function(form) {
        $.ajax({
			method: "POST",
            url: "{{ url('deletepreviouslyinstalled')}}",
...
$('#edit').on('show.bs.modal', function(e) {
	...
	$.ajax({
		type: "POST",
		url: "{{ url('previouslyinstalledbyid')}}",
		data: {id:id},
		dataType: "JSON",
		success: function(data) {
			$("#editid").val(id);
